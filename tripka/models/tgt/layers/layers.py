...
from .utils import trimat_to_mask
...
class EGT_Attention(nn.Module):
    ...
    def forward(self, h, e, mask):
        ...
        h_ln = self.mha_ln_h(h) # norm
        e_ln = self.mha_ln_e(e) # norm
        ...
        # Multi-head attention， self._dot_dim = node_width // nums_heads
        ...
        Q = Q * self._scale_factor  # self._scale_factor = self._dot_dim ** -0.5
        
        gates = torch.sigmoid(G+mask)   # edges as gates
        H_hat = torch.einsum('bldh,bmdh->blmh', Q, K) + E   # [b, nodes, nodes, num_heads], edges as bias
        A_tild = F.softmax(H_hat+mask, dim=2) * gates       # [b, nodes, nodes, num_heads]
...
class TGT_Layer(nn.Module):
    ...
    def forward(self, g):
        h, e, mask = g.h, g.e, g.mask   # [b, nodes, node_width], [b, nodes, nodes, 256], [b, nodes, nodes, 1] 
        triplet_matrix = g.triplet_matrix
        
        h_r1, e_r1 = h, e
        h, e, attn_weights  = self.update(h, e, mask)
        ...
        if self.edge_update:
            ...
            if self._triplet_update:
                e_rt = e
                e = self.tria(e, mask, triplet_matrix)
                e = self.drop_path(e)
                e.add_(e_rt)
