...
def stack_tensor_2d(tensor_list):
    assert len(tensor_list[0].shape) == 2
    return torch.stack([F.pad(tensor, (0, 0, 0, max(tensor.size(0) for tensor in tensor_list) - tensor.size(0))) 
                      for tensor in tensor_list])
    
def stack_tensor_1d(tensor_list):
    assert len(tensor_list[0].shape) == 1
    max_n = max(tensor.size(0) for tensor in tensor_list)
    return torch.stack([F.pad(tensor, (0, max_n - tensor.size(0))) for tensor in tensor_list])

def stack_tensor_2d_2(tensor_list):
    assert len(tensor_list[0].shape) == 2 and tensor_list[0].shape[0] == tensor_list[0].shape[1]
    max_n = max(tensor.size(0) for tensor in tensor_list)

    return torch.stack([F.pad(tensor, (0, max_n - tensor.size(1), 0, max_n - tensor.size(0))) 
                        for tensor in tensor_list])
